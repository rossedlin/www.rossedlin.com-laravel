<?php
/**
 * Created by PhpStorm.
 * User: Ross Edlin
 * Date: 03/09/2017
 * Time: 12:22
 *
 * @var App\Objects\ColorPalette\Item $colorPaletteItem
 */
?>
<style>
    .color
    {
        width:  400px;
        height: 100px;
    }
</style>

<div class="color-palette masonry-grid-item col-sm-6 col-lg-4 g-mb-30">
    <!-- Blog Grid Modern Blocks -->
    <article class="u-shadow-v21 u-shadow-v21--hover g-transition-0_3">

        <?php foreach ($colorPaletteItem->getColors() as $color)
        {
            ?>
            <div class="color" style="background-color: <?= $color ?>"><?= $color ?></div>
        <?php } ?>

        <div class="g-bg-white g-pa-30 g-rounded-bottom-5">
            <ul class="list-inline g-color-gray-dark-v4 g-font-weight-600 g-font-size-12">
                <li class="list-inline-item mr-0">Alex Teseira</li>
                <li class="list-inline-item mx-2">&#183;</li>
                <li class="list-inline-item">7 June 2017</li>
            </ul>

            <h2 class="h5 g-color-black g-font-weight-600 mb-4">
                <a class="u-link-v5 g-color-black g-color-primary--hover g-cursor-pointer" href="/portfolio/cryslo">Cryslo</a>
            </h2>
            <p>
                <a class="u-link-v5 g-color-black g-color-primary--hover g-cursor-pointer"
                   href="http://www.cryslo.com/">www.cryslo.com</a>
            </p>

            <ul class="list-inline g-font-size-12 mb-0">
                <li class="list-inline-item g-mb-10">
                    <a class="u-tags-v1 g-color-teal g-bg-teal-opacity-0_1 g-bg-teal--hover g-color-white--hover g-rounded-50 g-py-4 g-px-15"
                       href="#">Art</a>
                </li>
                <li class="list-inline-item g-mb-10">
                    <a class="u-tags-v1 g-color-yellow g-bg-yellow-opacity-0_1 g-bg-yellow--hover g-color-white--hover g-rounded-50 g-py-4 g-px-15"
                       href="#">Design</a>
                </li>
            </ul>
        </div>
    </article>
    <!-- End Blog Grid Modern Blocks -->
</div>

